...
# Default False: CDP can fail with HTTP 502 on /json/version (port conflict, proxy); standard Playwright uses the same profile dir as CDP (see browser_persistent_data_dirname).
...
def browser_persistent_data_dirname() -> str:
    """Folder name under browser_data/ for persistent login state (matches CDP user-data naming)."""
    return f"cdp_{USER_DATA_DIR % PLATFORM}"


# When True (or env XHS_DEBUG_SELFINFO=1), log selfinfo HTTP status and truncated body to debug pong=False.
DEBUG_XHS_SELFINFO = os.getenv("XHS_DEBUG_SELFINFO", "").lower() in ("1", "true", "yes")
